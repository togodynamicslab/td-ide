...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<'ul'>) {
  return (
    <ul
      ...
      className={cn(
        'mx-1 flex min-w-0 translate-x-px flex-col gap-1 border-l border-td-border px-1 py-0.5',
        'group-data-[collapsible=icon]:hidden',
        className
      )}
      {...props}
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        'flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-td-text-tertiary outline-none hover:bg-td-hover hover:text-td-text focus-visible:ring-1 focus-visible:ring-td-accent disabled:pointer-events-none disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:h-4 [&>svg]:w-4 [&>svg]:shrink-0',
